...
def write_worklog(worklog: Worklog, *, scope: str = "") -> str:
    ...
    lines: list[str] = [
        ...
        "else is regenerated by `latexa11y scan`, so edits outside `description`,",
...
def _render_entry(entry: Entry) -> list[str]:
    ...
    lines.append("")
    lines.append(entry.description if entry.description else "<!-- write the short description here -->")
    lines.append("")
    lines.append("### long")
    lines.append("")
    lines.append(
